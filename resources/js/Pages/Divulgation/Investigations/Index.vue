...
<template>
	...
	<div class="w-full flex flex-col items-center justify-center sm:px-60 sm:py-10">
			...
			<div
				...
				<div class="w-1/2 p-4">
					<h5 class="text-neutral-500 font-sans">
						{{ props.investigations[ 0 ].publicated_at }}
					</h5>
					<h1 class="text-4xl font-extrabold font-sans mt-2 text-neutral-700">
						{{ props.investigations[ 0 ].title }}
					</h1>
					<h2 class="text-lg font-medium text-neutral-600 mt-2">
						{{ props.investigations[ 0 ].short_description }}
					</h2>
					<button v-if=" props.investigations[ 0 ].file "
						@click="handleOpenPdfModal( props.investigations[ 0 ].file.path )"
				...
				<div class="w-1/2 h-80 overflow-hidden object-contain flex justify-center">
					<div v-if=" props.investigations[ 0 ].file !== null " class=" scale-90 -m-[20px]">
						<PdfThumbnail :url=" props.investigations[ 0 ].file.path " />
					</div>
		...
		<section class="w-full pb-6">
			<h1 class="text-xl text-neutral-600 font-bold">Investigaciones recientes</h1>
		</section>
		<section class="w-full grid grid-cols-1 md:grid-cols-3 gap-10">
			<div v-for="      investigation in props.investigations      " :key=" investigation.index "
				...
				<div class="w-full h-60 mb-2 overflow-hidden object-contain flex justify-center items-start">
					<div v-if=" investigation.file !== null " class="scale-[60%] -m-[160px]">
				...
				<div class="w-full h-52 flex flex-col items-start justify-between p-4 gap-0">
					...
					<h2 class="text-md font-medium text-neutral-600">{{ investigation.short_description.substr( 0, 50 )
						}}</h2>
					<button v-if=" investigation.file " @click="handleOpenPdfModal( investigation.file.path )"
