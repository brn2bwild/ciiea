	...
	<div class="w-full flex flex-col items-center justify-center sm:px-60 sm:py-10">
		<section class="w-full py-10 pb-28">
			<div
				...
				<div class="w-1/2 p-4">
					...
					<button v-if=" props.magazines[ 0 ].file "
						@click="handleOpenPdfModal( props.magazines[ 0 ].file.path )"
				...
				<div class="w-1/2 h-80 overflow-hidden object-contain flex justify-center">
					<div v-if=" props.magazines[ 0 ].file !== null " class=" scale-90 -m-[20px]">
		...
		<section class="w-full grid grid-cols-1 md:grid-cols-3 gap-10">
			<div v-for="                magazine in props.magazines                " :key=" magazine.index "
				...
				<div class="w-full h-48 flex flex-col items-start justify-between p-4 gap-0">
					<h5 class="text-neutral-500 font-sans text-sm">{{ magazine.publicated_at }}</h5>
					<h1 class="text-xl font-extrabold font-sans">
						{{ magazine.name }}
					</h1>
					<button v-if=" magazine.file " @click="handleOpenPdfModal( magazine.file.path )"
