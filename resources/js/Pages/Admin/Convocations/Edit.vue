...
const convocationForm = useForm({
	...
	time: new Date(props.convocation.date_time).toLocaleTimeString().substring(-1, 5),
		...
		<section class="w-full bg-white rounded p-8 flex justify-between">
			<form @submit.prevent=" handleUpdateConvocation " class="w-3/4">
				<div>
					...
					<TextInput id="name" type="text" class="mt-1 block w-full" v-model=" convocationForm.name " required
						autofocus autocomplete="convocation_name" />
				...
				<div class="mt-4 flex justify-between gap-x-4">
					<div class="w-1/2">
						...
						<TextInput id="convocation_date" type="date" class="mt-1 block w-full"
							v-model=" convocationForm.date " required />
						<InputError class="mt-2" :message=" convocationForm.errors.date " />
					</div>
					<div class="w-1/2">
						<InputLabel for="time" value="Hora" />
						<TextInput id="convocation_time" type="time" class="mt-1 block w-full"
							v-model=" convocationForm.time " required />
						<InputError class="mt-2" :message=" convocationForm.errors.time " />
					</div>
				</div>
				<div class="mt-4">
					<InputLabel for="location" value="Locación" />
					<TextInput id="location" type="text" class="mt-1 block w-full" v-model=" convocationForm.location "
						required autofocus autocomplete="convocation_location" />
				...
				<div class="mt-4">
					...
					<TextInput id="description" type="text" class="mt-1 block w-full"
						v-model=" convocationForm.description " required autofocus
