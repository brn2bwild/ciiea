...
<template>
	...
	<div class="flex justify-between items-center px-8">
		...
		<button @click="showCreateModal = true" class="bg-sky-500 rounded-full px-4 py-2 text-neutral-50 font-medium hover:bg-sky-700 transition-all duration-200">Nuevo administrador</button>
	</div>

	<div class="w-full p-8">
		<section class="grid grid-cols-1 md:grid-cols-4 gap-4">
			<ResourceCard v-for=" administrator in administrators " :key=" administrator.index "
				@open-delete-modal="handleOpenDeleteModal( administrator.id )" :edit-route=" route( 'admin.administrators.edit', administrator.id ) ">
		...
		<div class="p-4">
			<form @submit.prevent=" handleCreateAdministrator()">
				<div class="mt-4">
					...
					<TextInput id="name" type="text" class="mt-1 block w-full" v-model=" administratorCreateForm.name " required autocomplete="name" />
					<InputError class="mt-2" :message=" administratorCreateForm.errors.name " />
				</div>
				<div class="mt-4">
					<InputLabel for="email" value="Correo electrónico" />
					<TextInput id="email" type="email" class="mt-1 block w-full" v-model=" administratorCreateForm.email " required autocomplete="email" />
					<InputError class="mt-2" :message=" administratorCreateForm.errors.email " />
				</div>
				<div class="mt-4">
					<InputLabel for="password" value="Contraseña" />
					<TextInput id="password" type="password" class="mt-1 block w-full" v-model=" administratorCreateForm.password " required autocomplete="password" />
					<InputError class="mt-2" :message=" administratorCreateForm.errors.password " />
				</div>
				<div class="mt-4">
					<InputLabel for="password_confirmation" value="Confirmar contraseña" />
					<TextInput id="password_confirmation" type="password" class="mt-1 block w-full" v-model=" administratorCreateForm.password_confirmation " required autocomplete="password_confirmation" />
				...
				<div class="w-full flex justify-end mt-8 gap-4">
					<PrimaryButton :class=" { 'opacity-25': administratorCreateForm.processing } " :disabled=" administratorCreateForm.processing ">
						Guardar
					</PrimaryButton>
